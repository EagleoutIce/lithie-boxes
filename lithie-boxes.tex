...
Darüber hinaus besitzt jede Box eine \say{Plain}-Variante (diese kann und wird auch vom jeweiligen Stil geändert). Für mehr Informationen über die Verwendung siehe \cmdref{SetBoxPlain} oder die \hyperref[sec:packetoptions]{Paketoption} \T{autostyle}.

\subsection{Nummerierung}

Die Nummerierung der Boxen kann durch \hyperref[sec:packetoptions]{Paketoptionen} kontrolliert werden. So sorgt \T{lockhigh} zum Beispiel dafür, dass die Nummerierung sich an der höheren Gliederungsebene im Dokument orientiert.
Diese Option wird auch in dieser Dokumentation verwendet. Mit \T{nolock} können diese Sperren deaktiviert werden.
...
Hinweis für die Beispiele: Die dokumentation macht \emph{nicht} von der \hyperref[sec:packetoptions]{Paketoptionen} \T{autostyle} gebrauch. Die Farben werden allerdings durch die Palette \say{\CurrentPalette}.
...
Dies ist der aktuelle Stand der Dokumentation.
